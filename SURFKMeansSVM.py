...
print("data read over, feature extra and each vocabulary make")
...
for i in range(trainlen):
    lb = trainlabel[i]
    if lb not in tmptrain:
        tmptrain[lb] = []
    tmptrain[lb].append(traindata[i])

fss = {}

for i in tmptrain:
    featureSet = np.float32([]).reshape(0, 64)
    for im in tmptrain[i]:
    ...
    compactness, labels, centers = cv2.kmeans(featureSet, wordCnt, bestLabels=None, criteria=criteria, attempts=20,
                                              flags=flags)

    fss[i] = (labels, centers)
...
for i in acs:
    print(intToAnimal[i]," predict accuracy ", acs[i]/eachlen)
print("total accuracy ", accuracy/tlen)
